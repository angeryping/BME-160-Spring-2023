...
class FastAreader:
    ...
    def doOpen (self):
        if self.fileName == '':  # replaced is with ==
            return sys.stdin
        else:
...
class OrfFinder():  # place in sequenceAnalysis when complete
    ...
    def findOrfs(self):
        ...
        foundStart = 0  # if a codon is found, will set to True for use in conditionals
        foundCodon = 0
        ...
        for frame in range(0,3):
            ...
            for p in range(frame,len(self.genome),3):
                ...
                if not foundCodon and codon in self.stopCodons:  # If no start codon was found but stop codon found. - dangling stop
                    start = 1
                    stop = p + 3
                    length = stop - start + 1
    ...
    def revOrfs(self):
        ...
        for frame in range(0, 3):  # Check  frames 1, 2, 3
            ...
            for p in range(frame, len(reverseComp), 3):
                codon = reverseComp[p:p + 3]  # The codon is 3 nucleotides.
                

                if codon == 'ATG':#in OrfFinder.startCodons:  # When start codon is found.
