...
class OrfFinder():  # place in sequenceAnalysis when complete
    ...
    def __init__(self, genome):
        """Initialize a list of lists of ORFs that contains the frame, start, stop, and length of an ORF. """
        self.genome = genome  # this is the sequence
        ...
        for frame in range(0,3):  # iterate through each frame
            ...
            for p in range(frame,len(self.genome),3):
                ...
                if codon in OrfFinder.stopCodons and foundStart:  # for normal situation when there is both start and stop codon so it is a normal ORF
                    ...
                    length = stop - start + 1
                    self.keepOrf((frame%3) + 1, start, stop, length)
                    startPositions = []
                    foundStart = False
                    foundCodon = True
    ...
    def reverseComplement(self):
        """ Create the reversed, complimentary strand of DNA."""
        return ''.join([self.nucComplement[base] for base in self.genome[::-1]])

    def revOrfs(self):
        """Find ORFs on the 5'-3' ('bottom") strand. Return it as a list of ORFs. """
        ...
        for frame in range(0, 3):  # Iterate through each frame
            ...
            if foundStart:  # dangling start
                self.keepOrf(-1 * ((frame % 3) + 1), startPositions[0] + 1, 1, stop - start + 1)
