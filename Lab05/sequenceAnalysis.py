...
class OrfFinder():  # place in sequenceAnalysis when complete
    ...
    def findOrfs(self):
        """Find ORFs on a 3'-5' strand. Return it as a list of ORFs."""
        starts = []  #store start positions

        for frame in range(3):
            for p in range(frame,len(self.genome),3):
                codon = self.genome[p: p + 3]
                if codon in OrfFinder.start_Codons:
                    starts.append(p)
                elif codon in OrfFinder.stop_Codons:
                    for startP in starts:  #iterate thru every start
                        geneCand = [startP, p + 2, p + 3 - startP, frame + 1]
                        geneCands.append(gendCand)
        pass
    def revComp (seq):
        reverse = ""
        comp = 
