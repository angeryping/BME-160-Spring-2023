...
class OrfFinder():  # place in sequenceAnalysis when complete
    ...
    def __init__(self, genome):
        """Set up a list of lists of ORFs that contains the frame, start, stop, and length of an ORF. """
        self.genome = genome
        self.orfs = []  # store ORFs frame, start, stop, and length, refer to keepOrf()

    def keepOrf(self, frame, start, stop, length):
        """ Collect and store attributes of the ORF for parsing later."""
    ...
    def findOrfs(self):
        ...
        for frame in range(0,3):  # iterate through each frame
            startPositions = []  # have a clear list when iterating through each frame
            foundStart = False
            foundCodon = False
            for p in range(frame,len(self.genome),3):
                codon = self.genome[p: p + 3]  # establish codon length as 3 characters long.

                if codon in OrfFinder.startCodons:  # for finding start codon in a frame
                    startPositions.append(p)
                    foundStart = True
                    foundCodon = True
                
                if codon in OrfFinder.stopCodons and foundStart:  # for normal situation when there is both start and stop codon so it is a normal ORF
                    start = (startPositions[0] + 1 - frame)
                    stop = (p + 3)
                    length = (stop - start + 1)
                ...
                if not foundCodon and codon in OrfFinder.stopCodons:  # for dangling stop situation  when there is a stop codon but no start codon in  seq
                    stop = (p + 3)
                    length = (stop - start + 1)
                    self.keepOrf((frame%3) + 1, start, (p + 3), length)
                    startPositions = []
                    foundCodon = True
                
            if foundStart:  # for dangling start situation when start codon found but no stop codon
                start = (startPositions[0] + 1)
                stop = (len(self.genome))
    ...
    #    comp = 
    #    for nuc in seq: 
    #        reverse += comp.get(nuc) 
    #    return reverse[::-1]
 
    #    print (revComp( "ATGTGCGCCTGAGGCGCTGC" ) )
    ...
    def revOrfs(self):
        """Create the reverse strand 5'-3' strand. Return it as a list of ORFs. """
        ...
        for frame in range(0, 3):  # Iterate through each frame
            ...
            for p in range(frame, len(reverseComp), 3):
                ...
                if codon in OrfFinder.startCodons:  # start codon
                    startPositions.append(p)
                    foundStart = True
                    foundCodon = True

                if codon in self.stopCodons and foundStart:  # normal sequence again
                    stop = (len(reverseComp) - startPositions[0])
                    start = (len(reverseComp) - (p+2))
                    if frame == 1: stop += 1
                    elif frame == 2: stop += 2
                    self.keepOrf(-1 * ((frame % 3) + 1), len(reverseComp) - startPositions[0], len(reverseComp) - (p+2), (len(reverseComp) - startPositions[0]) - (len(reverseComp) - (p+2)) + 1)
                    startPositions = []
                    foundStart = False
                    foundCodon = True

                if not foundCodon and codon in self.stopCodons:  #dangling stop
                    self.keepOrf(-1 * ((frame % 3) + 1), len(reverseComp) - p - 2, len(reverseComp), (len(reverseComp) - startPositions[0]) - (len(reverseComp) - (p+2)) + 1)
                    startPositions = []
                    foundCodon = True
            if foundStart:  # dangling start
                self.keepOrf(-1 * ((frame % 3) + 1), startPositions[0] + 1, 1, (len(reverseComp) - startPositions[0]) - (len(reverseComp) - (p+2)) + 1)
